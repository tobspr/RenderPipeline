...
vec3 get_probe_at(ivec3 coord, vec3 nrm) {
    ...
    if (offs > bake_mesh_probecount.x * bake_mesh_probecount.y * bake_mesh_probecount.z || offs < 0) {
               return vec3(1, 0, 0);
    }

    ivec2 base_coord = ivec2((offs % bake_divisor) * 6, offs / bake_divisor);
    vec3 baked = vec3(0);
    baked += texelFetch(GIDataTexture, ivec2(0, 0) + base_coord, 0).xyz * max(0, dot(nrm, vec3(1, 0, 0)));
    baked += texelFetch(GIDataTexture, ivec2(1, 0) + base_coord, 0).xyz * max(0, dot(nrm, vec3(-1, 0, 0)));
    baked += texelFetch(GIDataTexture, ivec2(2, 0) + base_coord, 0).xyz * max(0, dot(nrm, vec3(0, 1, 0)));
