...
vec3 importance_sample_ggx(vec2 xi, float roughness)
{
    ...
    float cos_theta = sqrt((1 - xi.y) / max(1e-10, 1 + (r_square * r_square - 1) * xi.y));
...
void main() {
    ...
    vec4 accum = vec4(0);
    uint num_samples = uint(256.0 + currentMip * 4096.0);

    vec2 local_seed = coord * 0.34238759 + 0.25389 * seed;
    ...
    for (uint i = 0; i < num_samples; ++i) {
        ...
        vec3 sampled = textureLod(SourceTex, vec2(theta / (2.0 * M_PI), phi / M_PI), 0).rgb;
        accum += vec4(sampled, 1);
    }
    ...
    vec4 oldValue = imageLoad(DestTex, ivec3(coord, currentFace));
    imageStore(DestTex, ivec3(coord, currentFace), vec4(accum.xyz + oldValue.xyz, 1.0));
}
